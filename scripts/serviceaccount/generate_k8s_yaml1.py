...
def create_k8s_yaml(data, output_dir, template_dir):
    ...
    for row in data:
        ...
        dir_path = os.path.join(output_dir, env, cluster, namespace)
        safe_makedirs(dir_path)

        # Generate Namespace per (env, cluster, namespace)
        ns_key = (env, cluster, namespace)
        ...
        # Generate Service Account per (env, cluster, namespace, sa_name)
        sa_key = (env, cluster, namespace, sa_name)
        ...
        role_key = (env, cluster, namespace, role)
        role_definitions[role_key]["verbs"].update(verbs_list)
        role_definitions[role_key]["api_groups"].update(api_group_list)

        role_binding_key = (env, cluster, namespace, role_binding)
        role_binding_definitions[role_binding_key].add((sa_name, role, tuple(api_group_list)))

    # Write consolidated Role YAML files per (env, cluster, namespace, role)
    for (env, cluster, namespace, role), role_data in role_definitions.items():
        role_file_path = os.path.join(output_dir, env, cluster, namespace, "role.yaml")
    ...
    # Write consolidated RoleBinding YAML files per (env, cluster, namespace, role_binding)
    for (env, cluster, namespace, role_binding), bindings in role_binding_definitions.items():
        role_binding_file_path = os.path.join(output_dir, env, cluster, namespace, "rolebinding.yaml")
